...
class OpenAIProvider(AiProvider):
    ...
    def _get_client(self):
        ...
        if self._client is None:
            try:
                ...
                masked_key = self.config.api_key[:8] + "..." if self.config.api_key else "None"
                logger.info(f"Creating OpenAI client: base_url={self.config.base_url}, api_key={masked_key}")
    ...
    async def chat(self, prompt: str) -> str:
        ...
        logger.info(f"AI request: model={self.config.model}, base_url={getattr(self.config, 'base_url', 'default')}, prompt_length={len(prompt)}")
        logger.debug(f"AI request data: {request_data}")
...
class DeepSeekProvider(OpenAIProvider):
    ...
    def __init__(self, config: AiConfig):
        base_url_from_config = config.base_url
        if not config.base_url:
            config.base_url = self.DEFAULT_BASE_URL
        if not config.model:
            config.model = self.DEFAULT_MODEL
        
        logger.info(f"DeepSeekProvider init: original_base_url={base_url_from_config}, resolved_base_url={config.base_url}")
